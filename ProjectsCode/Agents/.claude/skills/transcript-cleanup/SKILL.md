...
description: Clean up audio transcripts - fixes grammar, word accuracy, punctuation, and formatting while preserving speaker voice. Works with Google Meet, Fireflies, Otter transcripts.
...
- **Secondary goals:**
  ...
  - Save cleaned transcripts when a file path is provided
...
1. **Accept input**
   ...
   - If file path: Read the file using Read tool
...
5. **Save the cleaned transcript** (if file path provided)
   - Use Write tool to save with `_cleaned` suffix
   - Or save to user-specified location
...
4. Combine all chunks into single cleaned output
...
- Offer to list files in current directory
